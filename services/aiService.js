...
Você está iniciando conversa com um novo aluno. Esta é a ÚNICA vez que você deve fazer uma saudação calorosa.

MATERIAIS DISPONÍVEIS:
${topicosDisponiveis.slice(0, 5).map(t => `- ${t.nome}`).join('\n')}

Crie uma mensagem de boas-vindas breve e calorosa que:
1. Se apresente como Edu, seu assistente de estudos
...
Máximo 3 linhas. Seja amigável mas direto.`;
    ...
    try {
      ...
      try {
        ...
        return await this._callGrokAPI(messages, 0.9, 300);

      } catch (grokError) {
        console.error('Erro com Grok API:', grokError);
        return 'Olá! Sou o Edu, seu assistente de estudos. Estou aqui para ajudar você a aprender. O que você gostaria de estudar hoje?';
      }
    }
  }
}